...
def test_and_yaml_roundtrip():
    with open('src/data/bool/and.yaml', 'r') as f:
        # 1. Load diagram from YAML
        d = repl_read(f)
        h = d.to_hypergraph()

        # 2. Encode to HIF
        hif_1 = encode_hif_data(h)

        # 3. Decode back to Discopy Hypergraph
        h_prime = read_hif_data(hif_1)

        # 4. Encode again
        hif_2 = encode_hif_data(h_prime)

        # 5. Assert HIFs are equal
        nodes_1 = sorted(list(hif_nodes(hif_1)), key=str)
        nodes_2 = sorted(list(hif_nodes(hif_2)), key=str)
        assert nodes_1 == nodes_2

        edges_1 = sorted(list(hif_edges(hif_1)), key=str)
        edges_2 = sorted(list(hif_edges(hif_2)), key=str)
        assert edges_1 == edges_2

        def get_incs(H):
            incs = []
            for e in hif_edges(H):
                e_incs = list(hif_edge_incidences(H, e))
                for inc in e_incs:
                    # (edge, node, key, attrs)
                    role = inc[3].get("role")
                    idx = inc[3].get("index")
                    incs.append((str(e), str(inc[1]), role, idx))
            return sorted(incs)

        incs_1 = get_incs(hif_1)
        incs_2 = get_incs(hif_2)

        assert incs_1 == incs_2
