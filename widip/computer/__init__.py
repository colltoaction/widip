...
# =============================================================================
# § 2.1 - Core Services and Metaprogramming
# =============================================================================
...
def service(name: str, dom: closed.Ty, cod: closed.Ty, **kwargs):
    ...
    def factory(*args, **f_kwargs):
        # Explicitly ensure we use closed.Ty to avoid monoidal.Ty issues
        d = closed.Ty(*dom)
        c = closed.Ty(*cod)
        @closed.Diagram.from_callable(d, c)
        def diag(*x):
            return ComputerBox(name, d, c, **kwargs)(*x)
        return diag
    return factory

class ComputerBox(closed.Box):
    """Base class for all computer boxes."""
...
# Service map for core.Computer metaprogramming
service_map = {
    "Copy": Copy,
    "Discard": Discard,
    "Swap": Swap,
...
TRUE = lambda: ComputerBox("⊤", closed.Ty(), Language, draw_as_spider=True)()
FALSE = lambda: ComputerBox("⊥", closed.Ty(), Language, draw_as_spider=True)()
IFTE = lambda: ComputerBox("ifte", Language @ Language @ Language, Language)
ProgramEq = lambda: ComputerBox("=?", Language @ Language, Language)

ZERO = lambda: ComputerBox("0", closed.Ty(), Language)()
...
ackermann_box = ComputerBox("ackermann", Language2, Language)
...
# =============================================================================
# § Bootstrapping and Make (The system's self-reconstruction)
# =============================================================================

def make_py():
    """Service to recreate its own Python code."""
    return ComputerBox("make.py", Language, Language)

def make_sh():
    """Service to recreate its own Shell code."""
    return ComputerBox("make.sh", Language, Language)

def make_yaml():
    """Service to recreate its own YAML code."""
    return ComputerBox("make.yaml", Language, Language)
