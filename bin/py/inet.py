    ...
    for u in inet.nodes:
        if is_active_wire(inet, u):
            ...
            if (c1tag, c2tag) == ("erase", "erase"):
                erase_erase.append(inet_eraera_rewrite_rule(inet, u))
            if (c1tag, c2tag) == ("construct", "duplicate"):
                construct_duplicate.append(inet_condup_rewrite_rule(inet, u))
            if (c1tag, c2tag) == ("duplicate", "construct"):
                construct_duplicate.append(inet_condup_rewrite_rule(inet, u))
            if (c1tag, c2tag) == ("duplicate", "duplicate"):
                concon_or_dupdup.append(inet_concon_or_dupdup_rewrite_rule(inet, u))
            if (c1tag, c2tag) == ("construct", "construct"):
                concon_or_dupdup.append(inet_concon_or_dupdup_rewrite_rule(inet, u))
            if (c1tag, c2tag) == ("construct", "erase"):
                condup_erase.append(inet_condup_erase_rewrite_rule(inet, u))
...
def inet_rewrite(inet: nx.MultiDiGraph, rule):
    ...
    ccs = nx.connected_components(nx.to_undirected(boundary))
    for cc in ccs:
        if len(cc) > 1:
            inet_merge_many_wires(inet, cc)
    return inet
