...
# Additional computational primitives
Language2 = closed.Ty("IO2")  # Second language type

class Partial(closed.Box):
    """Partial application box"""
    def __init__(self, dom, cod):
        super().__init__("Partial", dom, cod)

class Merge(closed.Box):
    """Merge box for combining computations"""
    def __init__(self, dom, cod):
        super().__init__("Merge", dom, cod)
